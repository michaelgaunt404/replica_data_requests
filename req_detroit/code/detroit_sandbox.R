...
generators_sf_ttl = generators_sf  %>%  select(!c(count_veh, vehicle_type)) %>%  unique()

generators_bg = generators %>%
  quick_buffer(radius = 50)

generators_bg_sf = replica_trip_origin_links %>%
  st_join(generators_bg) %>%
  st_drop_geometry() %>%
  filter(!is.na(Name)) %>%
  group_by(Name, vehicle_type) %>%
  summarise(count_veh = sum(count)) %>%
...
generators_bg_sf_hd = generators_bg_sf %>%  filter(vehicle_type == "HEAVY_COMMERCIAL")
generators_bg_sf_md = generators_bg_sf %>%  filter(vehicle_type != "HEAVY_COMMERCIAL")
generators_bg_sf_ttl = generators_bg_sf %>%  select(!c(count_veh, vehicle_type)) %>%  unique()


replica_trip_origin_links_sa = replica_trip_origin_links %>%
  st_filter(acquired_sa_polys)

# replica_trip_origin_links_sa_comb = replica_trip_origin_links_sa %>%
#   group_by(stableEdgeId) %>%
...
temp_map = mapview(generators_sf_ttl, zcol = "count_ttl", layer.name = "Generators - Combined Duty") +
  ...
  mapview(replica_trip_origin_links_sa_comb, zcol = "count", layer.name = "Raw Freight Origin Links"
                     ,cex = "count")
...
generators_bg_sf_ttl

temp_map = mapview(generators_sf_ttl, zcol = "count_ttl", layer.name = "Generators - Combined Duty") +
  mapview(generators_bg_sf_ttl, zcol = "count_ttl", layer.name = "Generators Big - Combined Duty") +
  mapview(replica_trip_origin_links_sa_comb, zcol = "count", layer.name = "Raw Freight Origin Links"
          ,cex = "count")
...
temp_map@map %>%
  htmlwidgets::saveWidget(here::here("req_detroit/docs", "detroit_gen_v3_sensitivity_map.html"))
