...
query_network_trip_using_bbox(
  bb_network_layer = 'req_everret/data/network_poly.gpkg'
  ,bb_sa_layer = 'req_everret/data/network_poly.gpkg'
  ...
  ,file_destination = "req_everret/data"
  ,max_record = Inf
  ,mode_type = c('COMMERCIAL', "PRIVATE_AUTO", "ON_DEMAND_AUTO")
...
links = make_network_link_layer(
  location = location
  ,folder = folder
  ,auto_save = T
)

points = make_network_centroid_layer(
  location = location
  ,folder = folder
  ,auto_save = T
)
...
data_temp = aggregate_network_links(
  ...
  ,network_object = links
)

temp_vis = viz_static_ntwrk_map_anl(
...
data_temp$agg_link %>% write_sf(here(location, folder, "network_links_2021.gpkg"))
htmlwidgets::saveWidget(temp_vis, here(location, folder, "vis_anl_2021.html"))
