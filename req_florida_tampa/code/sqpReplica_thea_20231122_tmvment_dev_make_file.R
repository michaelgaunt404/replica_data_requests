...
folder = 'mvmnt_data_20231214_brndnPrky_fix'
...
test_object = tmp_1 %>%
  ...
  mutate(type = rep(c("agg_link", "agg_link_flag", "agg_link_vehicle_type", "agg_link_vehicle_and_trip_type", "agg_link_vehicle_type_origin"), 1)
         ,agg_col = rep(c(NA, "flag_trip_type", "vehicle_type", "map_var", NA), 1))

yolo = test_object %>%
  filter(type == "agg_link_vehicle_and_trip_type") %>%
  .[1, 3] %>%
